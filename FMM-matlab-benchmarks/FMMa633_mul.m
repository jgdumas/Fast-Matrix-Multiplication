function C = FMMa633_mul(A, B, nmin, peeling, level)
...
%          level  : logging level.
  ...
  if nargin < 5                   % Min level for verbose output
    level = min([floor(log(m/nmin)/log(6)),floor(log(k/nmin)/log(3)),floor(log(n/nmin)/log(3))]);
  end
  C=FMMa633_mul_6_3_3(A,B,nmin,peeling,level);
end
